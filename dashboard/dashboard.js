...
async function showDashboard() {
  ...
  try {
    ...
    const notebooks = [];
    let useSupabaseFallback = !gToken;
    ...
    if (gToken) {
      ...
      if (!folderRes.ok) {
          console.warn('Drive access failed, falling back to Supabase', folderRes.status);
        ...
        if (filesData.files) {
            for (const f of filesData.files) {
                if (f.name.endsWith('.json') || (f.appProperties && f.appProperties.inkwellId)) {
                    try {
                        const fRes = await fetch(`https://www.googleapis.com/drive/v3/files/${f.id}?alt=media`, {
                            headers: { Authorization: `Bearer ${gToken}` }
                        });
                        const json = await fRes.json();
                        notebooks.push(json);
                    } catch (e) {
                        console.warn('Could not parse notebook', f.name);
                    }
                }
            }
        }
    ...
    if (useSupabaseFallback) {
      // Fetch from Supabase
      const res = await fetch(`${SUPABASE_URL}/rest/v1/notebooks?owner_id=eq.${encodeURIComponent(session.userId)}&select=id,title,updated_at,notebook_json`, {
        headers: {
          'apikey': SUPABASE_KEY,
      ...
      if (!res.ok) {
        ...
        throw new Error('Fehler beim Laden von Supabase');
      }
      const data = await res.json();
      for (const row of (data || [])) {
        if (row.notebook_json) notebooks.push(row.notebook_json);
      }
    }
...
document.getElementById('auth-connect-drive-web').addEventListener('click', () => {
  ...
  const url = `${SUPABASE_URL}/auth/v1/authorize?provider=google&redirect_to=${encodeURIComponent(currentUrl)}&prompt=consent%20select_account&scopes=https://www.googleapis.com/auth/drive.file`;
